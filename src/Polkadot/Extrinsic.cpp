...
static std::map<const std::string, Data> polkadotCallIndices = {
    {balanceTransfer, Data{0x05, 0x00}}, {utilityBatch, Data{0x1a, 0x00}},
    {stakingBond, Data{0x07, 0x00}},     {stakingBondExtra, Data{0x07, 0x01}},
    {stakingUnbond, Data{0x07, 0x02}},   {stakingWithdrawUnbond, Data{0x07, 0x03}},
    {stakingNominate, Data{0x07, 0x05}}, {stakingChill, Data{0x07, 0x06}},
...
static std::map<const std::string, Data> kusamaCallIndices = {
    ...
    {stakingChill, Data{0x06, 0x06}},          {stakingRebond, Data{0x06, 0x13}},
...
Data Extrinsic::encodeStakingCall(const Proto::Staking& staking, TWSS58AddressType network,
    ...
    case Proto::Staking::kRebondAndNominate: {
        // encode call1
        Data call1;
        {
            auto staking1 = Proto::Staking();
            auto rebond = staking1.mutable_rebond();
            rebond->set_value(staking.bond_and_nominate().value());
            // recursive call
            call1 = encodeStakingCall(staking1, network, specVersion);
        }
        ...
        Data call2;
        {
            auto staking2 = Proto::Staking();
            auto nominate = staking2.mutable_nominate();
            for (auto i = 0; i < staking.rebond_and_nominate().nominators_size(); ++i) {
                nominate->add_nominators(staking.rebond_and_nominate().nominators(i));
            }
            // recursive call
            call2 = encodeStakingCall(staking2, network, specVersion);
        }
